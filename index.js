...
const { imunes } = require('./src/menus/travas/imunes')
const { dbtravas } = require('./src/menus/travas/database')
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} Ja foi tarde Bakaaa`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[🔒] ESTE PEDIDO É ESPECIFICO PARA USUARIO MOD Pepy BOT*',
					...
					userB: `Desculpe, irmã. Você não está registrado como amigo de Pepy. Registre-se para fazer amizade com o bot Pepy por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Pepy|17\n\n-------------------Pepy Bot------------------ `,
			...
			const premium = ["5519999897566@s.whatsapp.net","553598455925@s.whatsapp.net","553598455925@s.whatsapp.net","5511949051934@s.whatsapp.net","5519999611134@s.whatsapp.net","5519989895444@s.whatsapp.net","5534993117667@s.whatsapp.net","553584271215@s.whatsapp.net"]
...
const get = require('got')
    ...
    client.sendMessage(id,`🎫ÚlTIMOS DADOS DO DISTRITO COVID-19 DA INDONÉSIA\n\n*Positif* = ${positif} \n*Sembuh* = ${sembuh} \n*Meninggal* = ${meninggal}\n*Dirawat* = ${dirawat}\n\n*Stay safe dan selalu gunakan masker saat berpergian*`, MessageType.text, { quoted: m } );
					...
					if (messagesC.includes("papai")){
			client.updatePresence(from, Presence.composing)
            tod = await getBuffer(`https://imgur.com/WB7sXXQ.jpg`)
            client.sendMessage(from, tod, image, { quoted: mek, caption: '--------------------❄ Pepy Bot ❄\n\n🛡Versão 5.0\n🛡STATUS: ON\n\n----------------------🛡DONO 🛡\n🛡NOME: Guilherme Portilho\n🛡IDADE: 17 anos\n🛡CIDADE: Vai vim me visitar? Kkkk\n🛡INSTA: hoshigaki_guilherme\n🛡WPP: http://wa.me/+5519999897566\n\n🛡 KIBA O BOT NÃO, OU TENHO ACESSO AO SEU IP, E SEU NÚMERO 🛡\n\n--------------------❄ Pepy Bot ❄'})                  
	}
	...
	if (messagesC.includes("fdp")){
	    ...
	    if (messagesC.includes("teamo")){
			client.updatePresence(from, Presence.composing)
			reply("eu tbm te mamo")
			reply("amo*")
	}
	   
			if (messagesC.includes("arrombado")){
			client.updatePresence(from, Presence.composing)
			reply("igual o cuzinho da sua mãe")
	}
			if (messagesC.includes("irys")){
			client.updatePresence(from, Presence.composing)
			reply("estão falando da minha namorada? ")
	}
			
						if (messagesC.includes("travar")){
			client.updatePresence(from, Presence.composing)
			reply("i ala Kkkkk o merdinha trava zap\n Vou te ensinar uma coisa bem legal\n 156.438.22 esse aqui é seu ip\n Quer seu CPF também?")
	}
			
						if (messagesC.includes("gui")){
			client.updatePresence(from, Presence.composing)
			reply("Se refira ao meu dono com Devido respeito!!")
			reply("Extremo ser de raça superiores, conhecido como Deus do novo mundo*")
			reply("Assim fica melhor 👍💯")
	}
			
						if (messagesC.includes("free")){
			client.updatePresence(from, Presence.composing)
			reply("Ala kkkkKKK o cara joga fornalha sem custos adicionaiskkkk")
	}   
	    			if (messagesC.includes("mily")){
			client.updatePresence(from, Presence.composing)
			reply("Ela é minha fã número 1° :3")
	}
	
	if (messagesC.includes(" kamily")){
			client.updatePresence(from, Presence.composing)
			reply("Ela é minha fã número 1° :3")
	}
	...
	if (messagesC.includes("bom")){
			client.updatePresence(from, Presence.composing)
			reply("Melhor agora com você aqui bb 😘")
	}
...
if (messagesC.includes("lorena")){
		...
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("falou o jogador de fogo grátis kkkkk")
	}
		...
		if (messagesC.includes("pepy")){
			client.updatePresence(from, Presence.composing)
			reply("Opa consagrado(a) no que  posso ajudar?")
	}
	
if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			reply("bot é sua jogabilidade, eu sou um ser além de sua compreensão!")
	}
...
//msc fim
			...
			switch(command) {
                ...
                case 'meny':
					...
					reply(mess.wait)
					client.sendMessage(from, 'Adicionando mensagem á  lista com sucesso' , text, { quoted: mek })
...
//TRAVAS
					...
					break
					case 'dbtravas':
					client.sendMessage(from, dbtravas(prefix), text)
					break
				...
				case 'n':
					...
					anu = await fetchJson(`https://st4rz.herokuapp.com/api/ytv2?url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `*Título do vídeo* : ${anu.title}\n\n*AGUARDE, ENVIANDO O VÍDEO.*`
					thumb = await getBuffer(anu.thumb)
					client.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}.mp4`, quoted: mek})
		...
		case 'iri':
			...
			break
				...
				case 'game':
					...
					setTimeout( () => {
					client.sendMessage(from, '_10 Outro segundoâ', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_20  Outros segundo', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					client.sendMessage(from, '_30 Outro segundo', text) // ur cods
					}, 1000) // 1000 = 1s,
					setTimeout( () => {
...
//@darkYT					
    ...
    break
				...
				case 'mia':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'i love you, mia ', quoted: mek })
					await limitAdd(sender) 	
					break
				case 'gcpf3':
					reply(mess.wait)
...
CRIADO DA CASE: â¬âðð¨ððð¦  â `
			        reply(placa)
					break
                case 'cpfg':
...
break
case 'hokages':
tujuh = fs.readFileSync('./bi/hokages.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tipobee':
tujuh = fs.readFileSync('./bi/tipobee.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'zoro':
tujuh = fs.readFileSync('./bi/zoro.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'deidara':
...
break
		   ...
		   await limitAdd(sender)
				...
				case 'meme':
					...
					ze =  ri[Math.floor(Math.random() * ri.length)];
					nye = await getBuffer(ze)
					client.sendMessage(from, nye, image, { caption: 'hoje eu tou comédianteï¸', quoted: mek })
					await limitAdd(sender) 	
					break 
			    case 'kbbi':
			...
			case 'instaimg':
				...
				break
                                case 'afk':
                                        ...
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *MÓDO AFK ${tels} NÃO PERTURBE SIM*`,
...
break
				...
				case 'setfoto':
                    ...
                    reply('Alterou com sucesso o icone do Grupo')
                    break			
                case 'level':
			    ...
			    case 'infonomor':
			        ...
			        break
			    case 'igstalk':
					if (args.length < 1) return reply('Nome de usuario!!')
					ige = body.slice(9)
					reply(mess.wait)
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`â­â*ã *TOTAL DE USUÃRIO Pepy BOT ð* ã\n\`\`\``
					no = 0
...
// recursos anime, random @Pepy domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Pepy domina *'})
                    break
                    case 'animemenu':
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'â­âã *PREMIUMð* ãâ*\n+ *NÃºmero* : \n+ *Status*: *DEATIVO*\n Te vejo para o próximo pedidoð\n*â°âã *posiÃ§ão* ãâ'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`â­âã *PREMIUMð* ãâ*\n+ *NÃºmero* : @${mentioned[0].split('@')[0]}\n+ *Status*: *DEATIVO*\n Te vejo para o próximo pedidoð\n*â°âã *posiÃ§ão* ãâ`, mentioned, true)
					client.sendMessage(from, mentioned)
				    }
					break
					case 'ichiadmin':
					tod = await getBuffer(`https://i.ibb.co/XDwBVDJ/1f2652c622fa.jpg`)
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*â­â*ã *ADMINBOT * ã\n*+ wa.me/5573998183889*â°â*ã *Pepy* ã**\n\n*_SE QUER SER ADMIN DO BOT Pepy_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*â­â*ã *PREMIUM USERð* ã\n*+ wa.me/551999897566*â°â*ã *Pepy* ã**\n\n*_SE QUER SER UM USUÃRIO PREMIUM DO Pepy BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Cara bate pra 2d kkkkkkkkkk printei'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ª'})
					break
				case 'malkova':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtbo5EcVSGj-IvEVznHIgMZ9vjFptZfvprtg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hehe boy'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'reislin':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hehehe'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
			...
			reply(mess.wait)
				...
				case 'limpar':
					...
					case 'clearall':
					if (!isOwner) return reply('Quem é você, acha que tem moral comigo?') 
					anu = await client.chats.all()
					client.setMaxListeners(25)
					for (let _ of anu) {
						client.deleteChat(_.jid)
					}
					reply('Todas as mensagens foram apagadas')
					break
        ...
        case 'promover':
                ...
                case 'bc3':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ EVENTOS ]\n\n${body.slice(4)}`})
						}
						reply('Transmissão enviada')
					} else {
						for (let _ of anu) {
